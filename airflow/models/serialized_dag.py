    ...
    * ``[scheduler] dag_dir_list_interval = 300`` (s):
    ...
    def write_dag(cls,
        ...
        if min_update_interval is not None:
            if session.query(literal(True)).filter(session.query(cls).filter(and_(cls.dag_id == dag.dag_id,
                     (timezone.utcnow() - timedelta(seconds=min_update_interval)) < cls.last_updated)).exists()).scalar():
                return
    ...
    def has_dag(cls, dag_id, session=None):
        ...
        :rtype: bool
        """
        return session.query(literal(True)).filter(session.query(cls).filter(cls.dag_id == dag_id).exists()).scalar()
