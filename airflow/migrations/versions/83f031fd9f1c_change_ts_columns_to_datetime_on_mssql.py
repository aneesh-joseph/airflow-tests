...
def is_table_empty(conn, table_name):
    result = conn.execute('select TOP 1 * from {table_name}'.format(table_name=table_name)).first() ==  None
    print("hello %s %s " % (table_name,result))
    return result
...
def drop_constraint(operator,table_name, constraint_dict):
    """
...
def create_constraint(operator, table_name, constraint_dict):
    """
...
def change_mssql_ts_column(conn,table_name,column_name):
...
def upgrade():
    ...
    if conn.dialect.name == "mssql":
        change_mssql_ts_column(conn,"dag_code", "last_updated")
        constraint_dict = get_table_constraints(conn, "rendered_task_instance_fields")
        drop_constraint(op,"rendered_task_instance_fields",constraint_dict)
        change_mssql_ts_column(conn,"rendered_task_instance_fields", "execution_date")
        create_constraint(op,  "rendered_task_instance_fields", constraint_dict)
